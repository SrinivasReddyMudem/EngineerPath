...
with st.sidebar:
    ...
    if purpose in PURPOSE_TO_CRITIC:
        st.caption(f"Runs '{PURPOSE_TO_CRITIC[purpose]}' automatically after generation — roughly doubles wait time.")
    show_raw = st.checkbox("Show raw JSON", value=False)
    run_clicked = st.button("Generate", type="primary", use_container_width=True)


def _render_result(agent_name: str, result):
    ...
    st.markdown(render(agent_name, result))
