...
from content_agents.core.config import get_enabled_agents
...
with st.sidebar:
    ...
    subject = st.text_input("Subject", value="Git Rebase", placeholder="e.g. Git Stash, Detached HEAD")
    enabled = get_enabled_agents()
    selected = st.multiselect("Agents to run", AGENT_NAMES, default=[a for a in AGENT_NAMES if a in enabled])
...
if run_clicked:
    if not subject.strip():
        ...
        for name, tab in zip(selected, tabs):
            with tab:
                ...
                if isinstance(result, AgentError):
                    ...
                    st.markdown(render(name, result))
                    ...
                    if quality_score:
                        with st.expander("Quality score (self-rated, regenerated if hook/analogy < 8)"):
                            cols = st.columns(3)
    ...
    st.info("Set a subject and agents in the sidebar, then click Generate.")
